...
object ExtMatrixFactorizationModelHelper extends Serializable {
  ...
  def recommendProductsForUsers(mdl: ALSModel, num: Int, blockSize: Int = 4096, lockCacheLevel: StorageLevel = StorageLevel.NONE): RDD[(Int, Array[Rating])] = {
    recommendForAll(mdl.rank, mdl.userFactors.rdd.map(x=>(x.getAs[Int]("id"), x.getAs[Seq[Float]]("features").map(_.toDouble).toArray)), mdl.itemFactors.rdd.map(x=>(x.getAs[Int]("id"), x.getAs[Seq[Float]]("features").map(_.toDouble).toArray)), num, blockSize, lockCacheLevel).map {
      case (user, top) =>
