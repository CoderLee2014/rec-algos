...
class FMModel(val task: Int,
  ...
  def load(sc: SparkContext, path: String): FMModel = {
    val data = FMModel.SaveLoadV1_0.Data(factorMatrix, weightVector, intercept, min, max, task)
    FMModel.load(sc, path)
  }

  def saveNative(options:Option[Config]): Unit = {
    val path = new Path(options.get.model_path + "/fm_model_" + options.get.train_date + "_" + options.get.version + "_native")
    val conf = new Configuration()
    val fs = FileSystem.get(conf)
    try{
      val os = fs.create(path, true)

      os.writeBoolean(true) //write k0
      os.writeBoolean(true) //write k1
      os.writeDouble(this.intercept)
      os.writeInt(this.numFactors)
      os.writeInt(this.numFeatures)
      os.writeInt(this.task)
      os.writeDouble(this.max)
      os.writeDouble(this.min)

      for(i <- 0 to (this.numFeatures-1)){
        os.writeDouble(weightVector.get(i))
      }
      //m_sum
      for(i <- 0 to (this.numFactors-1)){
        os.writeDouble(0.0)
      }
      //m_sum_sqr
      for(i <- 0 to (this.numFactors-1)){
      ...
      for(i <- 0 to (this.factorMatrix.numRows-1)){
        for(j <- 0 to (this.factorMatrix.numCols -1)){
          os.writeDouble(this.factorMatrix(i,j))
        }
      }
      os.flush()
      fs.close()
      println("Finished saving fm model natively.")
    }catch{
      case e: Exception =>
        println("Save model exception.")
    }
...
object FMModel extends Loader[FMModel] {
  ...
  private object SaveLoadV1_0 {
    ...
    def save(sc: SparkContext, path: String, data: Data): Unit = {
      ...
      dataRDD.write.mode("overwrite").parquet(dataPath(path))
    }
  ...
  def loadNative(options:Option[Config]): FMModel = {
    val path = new Path(options.get.model_path + "/fm_model_" + options.get.train_date + "_" + options.get.version + "_native")
    val conf = new Configuration()
    val fs = FileSystem.get(conf)
      val os = fs.open(path)

      val k0 = os.readBoolean() //write k0
      val k1 = os.readBoolean() //write k1
      val intercept = os.readDouble()
      val numFactors = os.readInt()
      val numFeatures = os.readInt()
      val task = os.readInt()
      val max = os.readDouble()
      val min = os.readDouble()
      val weightVector = new Array[Double](numFeatures)
      val m_sum = new Array[Double](numFactors)
      val m_sum_sqr = new Array[Double](numFactors)
      var factorMatrix = Array[Double]()

      for(i <- 0 to (numFeatures-1)){
        weightVector(i) = os.readDouble()
      }

    //m_sum
    for(i <- 0 to (numFactors-1)){
       m_sum(i) = os.readDouble()
    }
    //m_sum_sqr
    for(i <- 0 to (numFactors-1)){
      m_sum_sqr(i) = os.readDouble()
    }
    //read FactorMatrix
    for(i <- 0 to (numFactors-1)){
        for(j <- 0 to (numFeatures-1)){
          factorMatrix :+= os.readDouble()
        }
      }
      fs.close()
      new FMModel(task, new DenseMatrix(numFactors, numFeatures, factorMatrix, true), Option(Vectors.dense(weightVector)), intercept, min, max)
  }
}
